...
def generate_multimodal_caption(image_path, category, tab_name):
    ...
    Промт чітко розділено на два режими: Меблевий гумор та Загальний/П'ятничний гумор.
    """
    ...
    try:
        ...
        # -------------------------------------------------------------
        # 2️⃣ ПОДІЛ ПРОМТУ НА ДВА ОКРЕМИХ КОНТЕКСТИ
        # -------------------------------------------------------------
        if is_furniture:
            # РЕЖИМ: Меблевий гумор
            prompt = (
                f"Ти — експерт з меблевого гумору, майстер меметики та креативний автор пабліку для меблевиків.\n"
                f"Аналізуй це зображення/мем крізь призму професії меблевика (конструктори, дизайнери, збірники, цеховики, монтажники).\n\n"
                f"🎯 КОНТЕКСТ ПУБЛІКАЦІЇ:\n"
                f"- Категорія / Тема: '{category}'\n"
                f"- Розділ: '{tab_name}'\n\n"
                f"🔥 ТЕМАТИКА ТА ФОКУС (МЕБЛЕВИЙ ГУМОР):\n"
                f"- Життєві болі та курйози: меблевий брак, зазори, непаралельні стіни, 'пляшучі' рівні, помилки в карті розкрою чи деталюванні, запізнення фасадів.\n"
                f"- Специфіка роботи: спілкування з замовниками ('мені тільки запитати', 'а можна на 1 см коротше вже готове?'), приколи в цеху та на монтажі, креслення 'на серветці', непідйомні заноси на 9 поверх без ліфта.\n"
                f"- Професійний сарказм та сленг (фурнітура, конфірмати, петлі, ДСП, кромка, присаджування, стільниці, розкрій).\n\n"
                f"🎯 ВИМОГИ ДО СТИЛЮ:\n"
                f"1. Жарт має бути живим, саркастичним та з точним влучанням у меблеву тему.\n"
                f"2. ЖОДНОГО дослівного перекладу! Адаптуй кожну мову під її меметичний та професійний сленг.\n"
                f"3. Використовуй формат punchline (коротко, влучно, з емодзі 🔨, 📐, 🪚, 😂).\n\n"
                f"⚠️ СУВОРІ ФОРМАТНІ ОБМЕЖЕННЯ:\n"
                f"- Не додавай вступів, лапок, пояснень від себе чи хештегів.\n"
                f"- Формат відповіді СТРОГО 3 абзаци з відповідними прапорами:\n\n"
                f"🇺🇦 [Жарт/коментар українською]\n\n"
                f"🇬🇧 [Жарт/коментар англійською]\n\n"
                f"🇩🇪 [Жарт/коментар німецькою]"
            )
        else:
            # РЕЖИМ: Загальний / П'ятничний гумор
            prompt = (
                f"Ти — легенда мемології та автор розважального пабліку про життєвий гумор і будні.\n"
                f"Подивись на це зображення/мем і придумай короткий, влучний, саркастичний коментар або життєву фразу.\n\n"
                f"🎯 КОНТЕКСТ ПУБЛІКАЦІЇ:\n"
                f"- Категорія / Тема: '{category}'\n"
                f"- Розділ: '{tab_name}'\n\n"
                f"🔥 ТЕМАТИКА ТА ФОКУС (ЖИТТЄВИЙ ТА П'ЯТНИЧНИЙ ГУМОР):\n"
                f"- Еволюція від мемів про 'очікування п'ятниці' та вихідні до запеклих буднів: дедлайни, важкі понеділки, режим 'не чіпайте мене', передчуття п'ятниці.\n"
                f"- Життєві болі 'до сліз': втома від роботи, побутові курйози, боротьба з будильником, сезонний настрій, абсурдність щоденних ситуацій.\n"
                f"- Максимальна релейтабельність (relatable humor), щоб кожен впізнав у мемі себе або колегу.\n\n"
                f"🎯 ВИМОГИ ДО СТИЛЮ:\n"
                f"1. Живий розмовний сленг, сарказм, іронія та гумор 'в яблучко'.\n"
                f"2. ЖОДНОГО дослівного перекладу! Створюй жарт окремо під культурні меми та сленг кожної мови.\n"
                f"3. Формат punchline з емодзі (☕, 😴, 🎉, 🍷, 😂).\n\n"
                f"⚠️ СУВОРІ ФОРМАТНІ ОБМЕЖЕННЯ:\n"
                f"- Не додавай вступів, лапок, пояснень від себе чи хештегів.\n"
                f"- Формат відповіді СТРОГО 3 абзаци з відповідними прапорами:\n\n"
                f"🇺🇦 [Жарт/коментар українською]\n\n"

...
# 🧠 ШІ ГЕНЕРАЦІЯ ОПИСУ С ЗБЕРЕЖЕННЯМ КОНТЕКСТУ СТОРІТЕЛІНГУ
def generate_ai_metadata(file_path, date_str, location_geo, previous_captions=None):
    """Аналізує візуальний контент через Gemini з урахуванням описів попередніх медіа."""
    ...
    # Формуємо блок контексту попередніх описів
    story_context = ""
    if previous_captions and len(previous_captions) > 0:
        # Беремо останні 3 описи для економії токенів
        recent_captions = "\n".join([f"- «{cap}»" for cap in previous_captions[-3:]])
        story_context = (
            "\nОПИСИ ПОПЕРЕДНІХ МЕДІА З ЦІЄЇ СЕРІЇ/ДНЯ:\n"
            f"{recent_captions}\n\n"
            "ВИМОГИ ДО ПОСЛІДОВНОСТІ (СНОПШОТ-СТОРІТЕЛІНГ):\n"
            "• Опис має бути ПРОДОВЖЕННЯМ або новим акцентом спільної історії.\n"
            "• КРИТИЧНО: НЕ повторюй ключові слова, однакові прикметники, заклики чи структуру речень з попередніх описів.\n"
            "• Змінюй фокус: якщо минулий опис був про емоції — цей зроби про деталь, атмосферу або динаміку кадрa.\n"
        )

    # Оновлений сучасний промпт
    prompt = (
        "Ти — експерт із вірусного travel-контенту TikTok та майстер сторітелінгу.\n"
        "Проаналізуй це зображення/кадр та згенеруй один короткий, влучний та естетичний підпис (1 речення або фраза).\n\n"
        f"Атмосферний контекст (НЕ згадуй ці дані прямо): Локація — {location}, Рік — {year}.\n"
        f"{story_context}\n"
        "ПРАВИЛА:\n"
        "1. Напиши текст виключно УКРАЇНСЬКОЮ мовою.\n"
        "2. Жодної банальщини («неймовірна краса», «чудовий день» тощо). Тільки живий, сучасний та інтригуючий вайб.\n"
        "3. ЗАБОРОНА: Не згадуй назву локації чи рік у тексті прямо.\n"

...
def get_exif_data(image_path):
    ...
    try:
        with Image.open(image_path) as img:
            ...
            # Резервний пошук в основній структурі EXIF
            if not date_str:
...
def get_intellectual_date(f_info, meta_date):
    ...
    if fn_date: return fn_date

    # Крок 2: Парсинг дати з метаданих файлу (EXIF / Video / Cloud)
    if meta_date:
    ...
    try:
        ...
        if valid_gdrive_dates: return min(valid_gdrive_dates)
    except: pass
...
def scan_folders_structure(drive_service, folder_id, top_category, drive_files_dict):
    """Рекурсивно знаходить файли з отриманням розширених хмарних метаданих (EXIF)."""
    ...
    while True:
        ...
        res = drive_service.files().list(
            q=q, 
            fields="nextPageToken, files(id, name, mimeType, createdTime, modifiedTime, imageMediaMetadata, videoMediaMetadata)", 
            pageSize=1000, 
...
def download_and_extract_meta(drive_service, file_id, f_info):
    """Витягує розширені метадані: спочатку з хмари Google Drive, за потреби — завантажує файл."""
    file_name = f_info['name']
    mime_type = f_info['mime']
    
    meta_date, lat, lon = None, None, None
    imm = f_info.get('imageMediaMetadata')
    vmm = f_info.get('videoMediaMetadata')
    
    # 1. Перевіряємо, чи Google Drive вже розпізнав метадані файлу в хмарі
    if imm:
        meta_date = imm.get('time')
        loc = imm.get('location')
        if loc:
            lat = loc.get('latitude')
            lon = loc.get('longitude')
    elif vmm:
        meta_date = vmm.get('time')

    fn_date = extract_date_from_filename(file_name)
    
    # Якщо є хмарні метадані АБО дата з імені — нам НЕ потрібно завантажувати файл!
    if imm or vmm or fn_date:
        file_dt = get_intellectual_date(f_info, meta_date)
        display_loc, group_loc = get_location_data_multilang(lat, lon)
        return file_dt.strftime('%d.%m.%Y'), display_loc, group_loc

    # 2. Фолбек: Завантажуємо файл, тільки якщо хмарні метадані абсолютно порожні
...
def main():
    ...
    # Обробка дельти (у пам'яті, без прямих Sheets-запитів у циклі)
    for f_id, f_info in drive_files.items():
        if f_id not in sheet_map:
            print(f"🆕 Знайдено новий файл: {f_info['name']}. Швидкий хмарний аналіз...")
            ...
            if existing["data"][1] != f_info['name'] or existing["data"][2] != f_info['category']:
                rows_to_update.append({
                    "range": f"'{TAB_NAME}'!B{existing['idx']}:C{existing['idx']}",
                    "values": [[f_info['name'], f_info['category']]]
                })

    for f_id, sheet_info in sheet_map.items():
        if f_id not in drive_files: ids_to_delete.append(sheet_info['idx'])

    # --- БЛОК БЕЗПЕЧНОГО ЗАПИСУ ДЕЛЬТИ З ОНОВЛЕННЯМ СЕСІЇ ---
    print("🔄 Перепідключаємось до Google Sheets для захисту від таймауту сесії...")
    _, sheets = get_services()

    # 1. Пакетне оновлення назв/категорій існуючих файлів
    if rows_to_update:
        print(f"🔄 Оновлюємо {len(rows_to_update)} змінених назв/категорій...")
        body = {'valueInputOption': 'RAW', 'data': rows_to_update}
        sheets.spreadsheets().values().batchUpdate(spreadsheetId=SPREADSHEET_ID, body=body).execute()

    # 2. Додавання нових рядків
    if rows_to_append:
    ...
    # 4. Збереження налаштувань папок
    ...
    print("✨ Реєстр меблів успішно синхронізовано та збагачено метаданими без таймаутів!")
